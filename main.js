...
let vals = {party: 1, lift: 0, bnum: 0}
let keymap = {}
let loadout = []
let brush
...
document.addEventListener('keydown', e => {                // menuing and D-pad
  keymap[e.code] = 1
  ...
  if(keymap['KeyQ']) {
    vals.bnum = rem(vals.bnum - 1, loadout.length)
    brush = loadout[vals.bnum]
  }
  if(keymap['KeyE']) {
    vals.bnum = (vals.bnum + 1) % loadout.length
    brush = loadout[vals.bnum]
  }
...
document.addEventListener('keyup', e => {                  // clear D-pad
  keymap[e.code] = 0
})

function keystuff() {                                      // activate D-pad
...
function show_loadout() {
  el('loaded').innerHTML = ''

  if(!loadout.length)
    loadout = brushes.slice(0,5)

  loadout.forEach((brush, i) => {
    if(brush.vals)
      vals = {...brush.vals, ...vals}
    el('loaded').innerHTML += `<span class="loaded" fwump="${i}" style="color: hsl(${[...brush.name].map(c=>c.charCodeAt(0)-90).reduce((acc,n)=>acc+n*7,0)}, 80%, 50%)">${brush.name}</span>`
  })
...
async function load(t) {
  ...
  let pickles = unslam_obj(brush_chunk?.data)
  if (pickles) {
    vals = pickles.vals
    vals.bnum ||= pickles.bnum // TODO: remove this
...
function add_my_chunks(cs) {
  ...
  let brush_chunk = {chunkType: 'paRd', data: slam_obj({vals, loadout: munge_loadout(loadout)})}
  cs.push(types_chunk)
  cs.push(brush_chunk)
  cs.push(iend)
  return cs
...
function untrick(o) {
  return Object.entries(o).map(p => p[1]?.includes?.('=>') ? [p[0], new Function('return ' + p[1])()] : p).reduce((acc, [k, v])=> {acc[k]=v; return acc}, {})
}
...
el('options').addEventListener('change', add_to_loadout)
el('loaded').addEventListener('click', rm_from_loadout)
